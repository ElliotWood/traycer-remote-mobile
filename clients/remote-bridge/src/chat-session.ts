...
export class ChatSession {
  ...
  constructor(opts: {
     ...
     * protocol gap separately (tickets/
     * `chat-send-accepts-and-swallows-null-settings`); this is the
     * client-side half.
     *
     * Required (not optional / defaulted) so a caller cannot forget to wire
     * it and land back on the silent-drop: an omitted callback is a compile
     * error, a defaulted one is a silent regression.
     */
  ...
  async sendMessage(text: string): Promise<ActionOutcome> {
    ...
    const settings =
      this.snapshot.chat.settings ?? (await this.resolveDefaultSettings());
    if (settings === null) {
      return {
        kind: "failed",
        reason:

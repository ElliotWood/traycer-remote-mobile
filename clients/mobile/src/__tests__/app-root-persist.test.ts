...
function fakeQuery(queryKey: readonly unknown[], status: "success" | "pending" | "error"): Query {
  return { queryKey, state: { status } } as unknown as Query;
}

describe("shouldDehydrateQuery", () => {
  it("persists a successful fleet query", () => {
    expect(shouldDehydrateQuery?.(fakeQuery(["mobile", "epic.listTasks", "fleet"], "success"))).toBe(true);
  });

  it("persists a successful comment-threads query", () => {
    expect(
      shouldDehydrateQuery?.(
        fakeQuery(["mobile", "epic.listCommentThreads", "e1", "ticket", "a1"], "success"),
      ),
    ).toBe(true);
  });

  it("excludes the lazy on-expand queries — not part of the empty-on-load bug", () => {
    expect(shouldDehydrateQuery?.(fakeQuery(["mobile", "snapshots.readSnapshotDiff", "h1", "h2"], "success"))).toBe(
      false,
    );
    expect(shouldDehydrateQuery?.(fakeQuery(["mobile", "workspace.readFile", "/w", "/f"], "success"))).toBe(false);
    expect(
      shouldDehydrateQuery?.(fakeQuery(["mobile", "agent.gui.getPlan", "e1", "c1", "p1"], "success")),
    ).toBe(false);
  });

  it("excludes a non-mobile-namespaced query key", () => {
    expect(shouldDehydrateQuery?.(fakeQuery(["epic.listTasks"], "success"))).toBe(false);
  });

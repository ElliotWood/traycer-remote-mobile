...
function toolCall(blockId: string, parentBlockId: string | null = null): ContentBlock {
  return {
    type: "tool_call",
    blockId,
    status: "completed",
...
function subagent(blockId: string, spawnToolCallId: string | null = null): ContentBlock {
  return {
    type: "subagent",
    blockId,
    status: "completed",
...
describe("partitionBlocks — the no-silent-drop partition", () => {
  it("classifies every block into exactly one bucket, dropped is empty", () => {
    const blocks: ContentBlock[] = [toolCall("t1"), fileChange("f1"), steer("s1")];
  ...
  it("puts the edit-suppressed tool_call in `suppressed`, not `dropped`", () => {
    ...
    const blocks: ContentBlock[] = [toolCall("t1"), fileChange("t1:0")];
    const result = partitionBlocks(blocks);
    expect(result.dropped).toEqual([]);
  ...
  it("puts the spawn-suppressed tool_call in `suppressed`, not `dropped`", () => {
    const blocks: ContentBlock[] = [toolCall("spawn1"), subagent("sa1", "spawn1")];
    const result = partitionBlocks(blocks);
    expect(result.dropped).toEqual([]);
  ...
  it("counts a nested child as rendered (visible, just indented), not dropped", () => {
    const blocks: ContentBlock[] = [subagent("sa1"), toolCall("t1", "sa1")];
    const result = partitionBlocks(blocks);
    expect(result.dropped).toEqual([]);
  ...
  it("suppression rules run over a combined snapshot+live-overlay set straddling the two", () => {
    ...
    const liveBlocks: ContentBlock[] = [toolCall("spawn-live")];

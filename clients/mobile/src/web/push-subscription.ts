 ...
 *
 * ## `surface-blocked`, and the attribute that was ABSENT on the one surface
 * ## that most needed it
 *
 * The paragraph above was the promise. On the Teams tab it was not kept, and the
 * gap was one caller away rather than anywhere in this file.
 *
 * `main.tsx` drove this function off the notification report hook behind
 * `if (outcome !== "granted") return;`. An embedded surface reports
 * `surface-blocked` — measured, never `granted` — so `ensurePushSubscription`
 * **was never called there at all** and `<html>` carried no `data-push`
 * attribute whatsoever. Absent is not one of the five states above: it is
 * indistinguishable from an old bundle with no push code, from a boot path that
 * threw before reaching this, and from push being fine but unmeasured. That is
 * the middle state — *unmeasured* — collapsed into silence, on the single
 * surface where the answer is permanent and the user can do nothing about it.
 *
 * So the guard is gone, this function is called on EVERY notification outcome,
 * and it is free to be: the permission gate below is the first line of
 * `resolve` and returns before any network call.
 *
 * WHAT IS MEASURED AND WHAT IS INFERRED, kept apart deliberately.
 *
 * - **Measured**, Chromium 1228, three arms, notifications granted to the
 *   origin: a cross-origin frame reads `Notification.permission === "denied"`,
 *   including with `allow="notifications *"` delegated from the parent. Top
 *   level and same-origin frame both read `granted`. That is the whole basis of
 *   the branch, and it is a reading this module shares with
 *   `notification-permission.ts` through the same `embedding.ts`.
 * - **Spec, not measured**: `subscribe({userVisibleOnly: true})` requires the
 *   notification grant, which is why the permission gate exists at all. A
 *   positive control for a real `subscribe()` is NOT obtainable here —
 *   automated Chromium refuses push subscription in every configuration tried,
 *   while both permission reads say granted — so no run in this environment can
 *   demonstrate a successful subscription to contrast against.
 *
 * The outcome is therefore worded as a statement about **the surface**, which is
 * the half that was measured, and not as a claim that a subscribe attempt was
 * made and refused. Nothing here rests on the unmeasurable link.
 *
 * The condition MIRRORS `notification-permission.ts` exactly — `denied` AND
 * cross-origin embedded, not "not granted AND embedded". Widening it would make
 * the two modules disagree: on an embedder that leaves the permission at
 * `default`, the notification shell still renders a real Enable offer, and a
 * push layer calling that surface blocked would contradict a banner the user is
 * looking at.
 *
...
export type PushSubscriptionOutcome =
   ...
   *
   * "Not yet asked" and "the user refused" both land here, and that is fine
   * because the next action is the same one: the notification offer. What must
   * NOT land here is an embedded surface — see `surface-blocked`.
   */
  | "permission"
  /**
   * The surface refuses the grant push depends on, and no user was asked.
   *
   * Distinct from `permission` for the reason `data-notifications` keeps
   * `surface-blocked` distinct from `denied`: one is a decision the user can
   * revisit, the other is a property of where the app is running, and sending
   * someone to browser settings for the second is advice that cannot work.
   */
  | "surface-blocked"
  /**
   * No `PushManager` here — an insecure origin, jsdom, or iOS Safari before
   * 16.4.
   *
   * THIS NO LONGER SAYS "a Teams tab", and the correction matters more than the
   * words. It listed one, and a Teams tab cannot reach this outcome by either
   * route that claimed it: the permission gate returns first, and the service
   * worker was separately MEASURED to register in a cross-origin frame in all
   * three arms with the same scope. So the old comment was not merely
   * unreachable, its premise was refuted. `unsupported` has a passing test, but
   * a test that reaches a branch by injection says nothing about which real
   * surface reaches it — which is exactly how a wrong attribution survives in a
   * green suite.
   */
  | "unsupported"
...
export interface EnsurePushSubscriptionOptions {
   ...
   * that the same `denied` reading produces two different outcomes — the same
   * seam, and the same reason, as `offerNotificationPermission`.
   */
...
async function resolve(
  ...
  // BEFORE the permission gate, because the reading it re-attributes is
  // `denied` — which that gate would otherwise report as `permission`, i.e. as
  // something the user can go and change. Same condition and same ordering as
  // `offerNotificationPermission`, so the two attributes never disagree about a
  // surface.
  if (permission === "denied" && isEmbedded()) return "surface-blocked";

  // Checked FIRST of the browser states, before any network call.
  // `subscribe({userVisibleOnly:true})` rejects outright without the
  // notification grant, so asking the service for its key beforehand would
  ...
  try {
    ...
    // registration itself is forbidden — a storage-DENIED context, which is a
    // stricter thing than a third-party frame.
    //
    // This deliberately no longer says "the Teams tab this same bundle serves".
    // That was measured FALSE: the worker registers in a cross-origin frame in
    // all three probe arms, at the same scope. A Teams tab reaches
    // `surface-blocked` above and never arrives here.
    return "unsupported";
  }

...
export const Browser = {
  /**
   * WAS a bare browser popup call with `noopener,noreferrer`, and the comment
   * above still calls this the laziest correct substitute. It was the laziest;
   * measurement is what removed "correct". With `noopener` that call returns
   * `null` whether it opened a page or was refused - the same observation for
   * opposite outcomes - so on the Teams tab this door could fail silently, and
   * DEVICE-CODE SIGN-IN is behind it (`auth-service.ts` opens
   * `verificationUriComplete` through here).
   *
   * `external-link.ts` owns the routing and the three-state reporting; this
   * stays a one-line delegation so the Capacitor alias surface is unchanged.
   */
  async open(options: { url: string }): Promise<void> {
    await openExternalUrl({ url: options.url });
  },

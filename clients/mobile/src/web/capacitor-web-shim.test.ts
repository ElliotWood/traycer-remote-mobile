...
/**
 * The BEHAVIOURAL half of the delegation. `external-link.test.ts` asserts
 * lexically that this file names the module; this drives the seam, so a shim
 * that imported it and still opened its own window fails here.
 */
describe("Browser.open", () => {
  ...
  it("routes through the registered Teams opener rather than a browser popup", async () => {
    const teamsOpen = vi.fn(async () => undefined);
    const openSpy = vi.fn(() => null);
    vi.stubGlobal("window", { ...globalThis.window, open: openSpy });
    setTeamsLinkOpener(teamsOpen);

    await Browser.open({ url: "https://example.test/a" });

    expect(teamsOpen).toHaveBeenCalledWith("https://example.test/a");
    // The negative half: routing that ALSO popped a window would open the link
    // twice for a Teams user.
    expect(openSpy).not.toHaveBeenCalled();
    vi.unstubAllGlobals();
  });

  it("falls back to a browser popup with no Teams opener registered", async () => {
    const openSpy = vi.fn(() => null);
    vi.stubGlobal("window", { ...globalThis.window, open: openSpy });

    await Browser.open({ url: "https://example.test/b" });

    expect(openSpy.mock.calls).toEqual([
      ["https://example.test/b", "_blank", "noopener,noreferrer"],
    ]);
    vi.unstubAllGlobals();
  });
});

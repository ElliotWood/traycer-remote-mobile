/**
 * Per-artifact-room Y.Doc replica registry.
 *
 * MOVED from `clients/mobile` when the Teams tab needed artifact bodies.
 * Framework-agnostic already, so the move is mechanical — but the reasoning
 * below is the reason it must not be reimplemented, and the reason the tab's
 * first pass was wrong: it no-op'd the artifact-room frames on the grounds
 * that "a frame this surface does not render must not take the screen down".
 * True for every other frame, and false for these — NOT RENDERING and NOT
 * RETAINING are different things, and the snapshot does not come twice.
 *
 *

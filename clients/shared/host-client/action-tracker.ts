 ...
 * semantics that is invisible from the contract and expensive to relearn.
 *
 * The result of an owner action, as the CALLER experiences it.
 *
 * `failed` means UNCONFIRMED, not "did not apply": the frame may have landed
 * on the host with no way for this process to learn that it did. A UI must
 * word it accordingly — "we could not confirm this" rather than "this did not
 * happen".
 */
export type ActionOutcome =
  ...
  | {
      ...
      /** Host closed the frame as malformed, the id no longer exists, the reconcile window expired, etc. */

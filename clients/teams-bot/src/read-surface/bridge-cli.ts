...
export function sendMessageAction(
   ...
   */
  // Explicit `| undefined`, not `?:` — the repo's `no-restricted-syntax` rule.
  // The docblock above already argues that omitting this is a DECISION
  // ("someone replying from the composer wants the bridge's `supervised`
  // default"); making the caller write `undefined` is that argument enforced
  // rather than stated. NOT an eslint-disable: the rule is right here.

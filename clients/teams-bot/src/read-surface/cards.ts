 ...
 * maps to the pipeline's authorising or closeout tools; this is the same rule
 * stated where a person can see it.
 *
 * Their filenames are deliberately NOT spelled out here. `cards.test.ts`
 * scans every non-test source file for those exact strings, and it matches
 * raw text — so naming them, even in prose, fails the scan. That is the
 * scanner being blunt rather than wrong: see its docblock.
 */
